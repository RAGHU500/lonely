...
package honda.hero.api;

import honda.hero.api.dto.DeviceJSON;
...
@API(name = "1", version = "1.0.0", context = "/1", tags = "1")
@DeviceType(value = "1")
public interface DeviceTypeService {
    ...
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/user/register"})
    ...
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/change-status"})
    Response changeStatus(@PathParam("deviceId") String deviceId,
                          @QueryParam("state") String state,
                          @Context HttpServletResponse response);

    /**
     * Retrieve Sensor data for the 1
     */
    @Path("device/stats/{deviceId}")
    @GET
    @Consumes("application/json")
    @Produces("application/json")
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/stats"})
    Response getSensorStats(@PathParam("deviceId") String deviceId, @QueryParam("from") long from,
                            @QueryParam("to") long to);

    @Path("/device/{device_id}")
    @DELETE
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/removeDevice"})
    Response removeDevice(@PathParam("device_id") String deviceId);

    @Path("/device/{device_id}")
    @PUT
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/updateDevice"})
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/devices"})
    Response getAllDevices();

    @Path("/device/download")
    @GET
    @Produces("application/zip")
    @Permission(scope = "1_user", permissions = {"/permission/admin/device-mgt/download"})
